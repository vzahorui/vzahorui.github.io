...
1. During its forward pass the Kalman Filter saves the history of its predicted and updated state estimates: {% raw %}<span>$\hat{x}_{k\mid k-1}$</span>{% endraw %} and {% raw %}<span>$\hat{x}_{k∣k}$</span>{% endraw %}, as well as covariances {% raw %}<span>$P_{k\mid k−1}$</span>{% endraw %} and {% raw %}<span>$P_{k\mid k}$</span>{% endraw %}.
...
The squared residual term is a measure of the squared error. It calculates the difference between the most accurate estimate of the state at time $k$ (the smoothed state {% raw %}<span>$\hat{x}_{k \mid N}$</span>{% endraw %}), and the one-step prediction of that state {% raw %}<span>$F_k \hat{x}_{k-1 \mid N}$</span>{% endraw %}. This term tells us how much our dynamics model was "wrong" at each step.
